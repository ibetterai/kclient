...
proxy, so TermHub's `localStorage['ib-theme']` (`'light' | 'dark' | 'system'`) is
directly readable. Separately, the audio/mic sidebar icons showed only a faint
...
2. `public/js/theme.js` — tiny head-blocking bootstrap: reads `ib-theme` fail-softly;
   `'system'`/absent → `prefers-color-scheme`; anything unreadable → dark (the historic
   look). Sets `data-theme` on `<html>`; live-follows `storage` events and OS scheme
...
- Standalone kclient (no TermHub) keeps working: no `ib-theme` key simply means
  OS preference with a dark default.
